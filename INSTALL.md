...
| 🔵 **Full** (`./install.sh`) | ~25 min | All 396 tools — adds Maps × CRM workflows, VRP routing, Receipt + Project-AP extractors, vendor follow-up loop, current-location detection | Daily-driver, sales/CSM/ops, AP automation, expense reporting |
...
A local MCP server that gives Claude Cowork **396 tools across 14 categories**: Gmail, Calendar, Drive, Sheets, Docs, Tasks, Contacts (with a CRM layer), Chat, Maps, cross-service Workflows, the Receipt + Project-Invoice extractors, and System health checks.
...
|---|---|---|---|
| 1. Core install | All 8 base Workspace tools (Gmail, Calendar, Drive, etc.) — **~396 tools** | 15 min | Yes |
...
You're done with Phase 1 — ~396 tools work right now.
